...
> 采购 · 销售 · 委外 · 库存 · 财务 · 退税 — 全链路数字化  
> **支持 AI 智能助手对话式操作**  
> v2.6.0 新增**销售退货财务层**：发票红冲 · 红字应收 · 退款 · 核销转移 · 退货联动 · 负数申报  
> v2.7.0 新增**预警提醒系统**：通知铃铛 · 事件埋点 · 规则配置 · 应收/应付账期预警  
> v2.8.0 重构**销售订单分支**：转直采 · 转外发（委外归口转外发）  
> 2026-09 认领材料改造：**转直采只关联采购订单（买成品，无视 BOM）**；委外行不关联采购——材料独立采购入库，委外领料出库才挂销售单；**生产管理下线**（批次追溯并入库存管理）
...
销售管理 → 销售订单（审核后明细行两路分流：转直采/转外发）→ 销售发货（两步化）→ 销售发票 → 应收账款 → 收款
采购管理 → 销售订单转采购（仅直采·买成品，无视 BOM）/ 独立材料采购订单 → 采购入库 → 采购发票 → 应付账款 → 付款
委外管理 → 销售订单转委外 → 委外订单（分工序/供料方式/领料出库挂销售单）→ 加工费应付
库存管理 → 收发存报表 / 库存流水 / 批次追溯 / 成品入库 / 原料入库 / 领料与原料出库 / 成品出库 / 盘点（独立菜单·盘盈盘亏）/ 采购红冲 / 销售退货
退税申报 → 申报期管理 / 发票关联 / 状态跟踪
AI 助手 → 右下角悬浮球随时对话 / 档案与库存查询 / 创建与审核单据 / 收付款 / 发票录入 / 操作手册问答（按菜单权限控制，全程留痕）
管理驾驶舱 → 现金收支 / 应收应付账龄 / 销售毛利分析（支持穿透查询）

（生产管理已于 2026-09 下线；历史数据表保留，批次追溯挪入库存管理）
```
...
| 文档 | 说明 |
|------|------|
| `docs/01-产品文档.md` | 产品功能概述 |
| `docs/02-项目文档.md` | 项目结构与数据设计 |
| `docs/03-开发文档.md` | 开发环境与规范 |
| `docs/04-操作手册.md` | 用户操作手册（面向业务人员） |
| `docs/05-营销一页纸.md` | 产品营销一页纸 |
| `docs/06-bots设计.md` | AI Bot 设计方案（未实施） |
...
```bash
# 后端全套（契约/状态机/边界/全流程/收发存v2/分支互斥，280+ 用例）
