        ...
        <el-form-item label="订单明细">
          <div style="width: 100%">
            ...
            <el-table :data="orderForm.items" border size="small" style="width: 100%; margin-top: 4px">
              ...
              <el-table-column label="来源销售单" width="140">
                <template #default="{ row }">
                  <span v-if="row.sales_order_no" style="color: #67c23a; font-size: 12px">{{ row.sales_order_no }}</span>
                  <span v-else style="color: #909399; font-size: 12px">备货</span>
                </template>
              </el-table-column>
...
function newItem() {
  return { material_id: null, product_id: null, material_code: '', material_name: '', unit: '', quantity: 1, unit_price: 0, tax_rate: 13, total_amount: 0, tax_amount: 0, total_amount_excl_tax: 0, sales_item_id: null, sales_order_no: '' }
}
...
function pickMaterial(m) {
  ...
  if (target) {
    ...
    target.sales_item_id = null
    target.sales_order_no = ''
