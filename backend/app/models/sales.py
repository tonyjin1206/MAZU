...
class SalesOrderItem(Base):
    ...
    supply_type = Column(String(16), default="己方提供", comment="委外供料方式(订单级): 己方提供/包工包料;仅转外发的行用")
...
class SalesDelivery(Base):
    ...
    warehouse_id = Column(Integer, ForeignKey("fd_warehouse.id"), nullable=True, comment="出库仓库(待出库时为空)")
    batch_no = Column(String(64), nullable=True, comment="出库批次号(待出库时为空)")
    quantity = Column(Float, nullable=False, comment="通知发货数量")
    delivered_qty = Column(Float, default=0, comment="已出库数量")
    unit_price = Column(Float, default=0, comment="含税单价(外币)")
