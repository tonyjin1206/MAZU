...
覆盖单据：采购需求(PR)、采购订单(PO)、销售订单(SO)。

✅ 已按 SP 流程重写（2026-08-28）：销售审核不再自动生成 MO，改走 re-produce 端点。
✅ 生产管理模块已下线（2026-09 合并 Sales_Purchase）：MO 状态机用例随之移除；
   PR 唯一 API 创建入口（production/to-requisition）已删，改为 re-produce 建 MO 后直插 PR 行。
"""
import pytest
...
def _make_mo(client, h, base):
    """SP 流程：销售订单审核 → re-produce 生成生产订单（后端端点保留），返回 MO id"""
...
def _make_pr(client, h, base):
    """建采购需求：re-produce 生成 MO 后直插 PR 行
    （API 创建入口 production/to-requisition 已随生产管理下线而删除）"""
    mo_id = _make_mo(client, h, base)
    from app.models.purchase import PurchaseRequisition
    db = SessionLocal()
    try:
        pr = PurchaseRequisition(
            requisition_no=f"PR-ST-{mo_id}", production_order_id=mo_id,
            product_id=base["prod"], quantity=100, status="待处理")
        db.add(pr)
        db.commit()
        return pr.id
    finally:
...
# 单据量从 状态×动作 全量组合 降到 动作数（PO 25→5、SO 21→3、PR 6→2）
_doc_pool = {}
...
def _run_matrix(client, h, base, doc_type, action, status, rule, maker):
    ...
    try:
        if action == "delete":
            ...
            resp = (client.put if action == "update"
