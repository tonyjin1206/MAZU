...
| 4 | 生产完工入库超量被接受 | 重复入库仍 200 | 400 拒绝 | ⚰️ **已失效**（2026-09 生产管理下线，完工入库端点已删除） |
...
| # | 需求 | 现状 | 期望 | 状态 |
|---|------|------|------|------|
| 001 | **委外订单要能开票** | 委外加工费现状走「委外订单审核→生成应付账款」，无独立委外加工费开票页面（原 production/ProcessingInvoices 创建接口写死 400，属半废弃页面） | 委外订单审核/完成后，能对加工费生成一张委外进项发票（关联委外单，记金额+税额，可抵扣进项税/进应付） | **待朋友考虑**（2026-09-01 峰子定：不做代码，财务方案留给朋友，见「认领材料改造」） |

> 说明：本次「认领材料改造」删生产管理模块时，会把已废弃的「加工费发票」页面（production/ProcessingInvoices）一并删除。真正的委外加工费记账入口 = 委外订单审核 → 生成应付账款。若朋友后续要「委外订单直接开票」，需按上表单独立项开发。
